...
The boundary doesn't rely on a ref or its position in the tree—it watches `document.body` for matching nodes directly—so you can wrap the `<Modal />` itself or wrap the surrounding `<Modal.Trigger />` / `<ModalContainer />`. Wrapping the `<Modal />` directly keeps the boundary scoped to that one modal.
