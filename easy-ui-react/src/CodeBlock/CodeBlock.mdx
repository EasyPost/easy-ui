...
If `<CodeBlock />` contains only one `<CodeBlock.Snippet />`, the `<CodeBlock />` will be in single language mode and not show a language selector.

<Canvas of={CodeBlockStories.SingleLanguageMode} />
