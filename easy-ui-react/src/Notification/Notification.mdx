...
Notifications can have custom placement on a per app basis. Use the `notificationPlacementProps` prop which is available on the `EasyUIProvider` component.

<Canvas of={NotificationStories.CustomPlacement} />

## Custom placement within container

Notifications by default render into `document.body` via a portal. To render notifications into a custom container, use the `htmlId` prop.
