...
- **`Popover.Title`** — a `Text` with the title id applied last, so it can't be overwritten. Defaults to `subtitle1`, a step down from `Modal`'s `heading4`, because a popover is a panel attached to a field rather than a page-blocking surface. `as="h2"` holds regardless of `variant` — heading level is document structure, size is visual.
