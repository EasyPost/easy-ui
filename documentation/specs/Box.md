...
| `text-decoration`                                                            | 4 inline styles + 2 modules                                 | Prop omission — the one gap `Box` itself causes, see below        |
...
Of the nine, three are worth closing, and only for reasons independent of how often the app hits them.
...
**`textDecoration` is the one gap `Box` creates rather than inherits.** `as="a"` applies `unstyled.link`, which sets `text-decoration: none` unconditionally, and no prop can restore it. Easy UI has no `Link` component and `Text` has no decoration property, so `<Box as="a">` inside body copy produces a link distinguishable from its surrounding text by color alone — a WCAG 1.4.1 failure with no escape hatch, in the component whose whole purpose is to be one. Volume is a red herring here: three of the four application sites merely strip an underline, which the reset already does for free. A two-value enum, non-responsive, one declaration.

**`overscrollBehavior` is coherent but premature.** `Box` already owns scroll containment through `overflow`, so `contain` is its natural partner and would cost one non-responsive declaration. One consumer is not enough to justify growing the API; revisit at two.

The remaining six should stay out, and in four cases the fix belongs somewhere else. Hover styling is an interactive `Card` variant, not a style prop — a `_hover={{ … }}` mechanism is the Primer and Chakra path this specification argues against, and it multiplies the stylesheet by every property allowed inside it. The `:global()` reach-throughs are a symptom of Easy UI components not accepting layout control; fixing those components removes the need, whereas a selector mechanism on `Box` would legitimize the reach-through. Three of the four `transform` uses are already expressible — `translate(-50%, -50%)` is `inset="0"` with `marginX`/`marginY="auto"`, and `translateY(-48px)` is `marginTop="-6"` — leaving one genuine visual effect. And the custom breakpoints are Bootstrap's 768/992, i.e. legacy rather than a requirement; a seventh breakpoint would add a `@media` block to every responsive property, roughly a sixth again of `Box`'s CSS.
